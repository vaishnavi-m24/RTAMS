  ...
  export class OwnerService {
    ...
    async create(createOwnerDto: CreateOwnerDto): Promise<{ id: number;firstName:string; lastName:string; message: string }> {
      try {
        //await this.ownerModel.create(createOwnerDto);
        const newOwner = await this.ownerModel.create(createOwnerDto);
        return { id: newOwner.id, firstName:newOwner.firstName,lastName:newOwner.lastName, 
